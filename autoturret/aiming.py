...
"""
    ...
    camera_view_physical_size = Point2D(
        x = 2 * depth * math.tan(fov_x / 2.0),
        y = 2 * depth * math.tan(fov_y / 2.0)
    )
    ...
    world_coordinate_point = Point3D(
        normalized_point.x * camera_view_physical_size.x,
        normalized_point.y * camera_view_physical_size.y,
        depth
    )
...
def world_coordinate_gun_to_gun_angles(point):
    ...
    """
    return PanTilt(
        pan=math.atan2(point.z, point.x)
        tilt=math.atan2(point.y, math.hypot(point.x, point.z))
    )
